...
execute if score Yellow ReadyCount = Yellow TeamCount if score Purple ReadyCount = Purple TeamCount run function ready:kit_check

# If any team not ready, announce status
execute unless score Yellow ReadyCount = Yellow TeamCount run tellraw @a [{"text":"Yellow is NOT ready! Missing: ","color":"red"},{"selector":"@a[team=Yellow,scores={Ready=0}]","color":"yellow"}]
execute unless score Purple ReadyCount = Purple TeamCount run tellraw @a [{"text":"Purple is NOT ready! Missing: ","color":"red"},{"selector":"@a[team=Purple,scores={Ready=0}]","color":"dark_purple"}]
